package com.hzdata.analytics.android.plugin.viewclick

import com.hzdata.analytics.android.plugin.ClassNameAnalytics
import com.hzdata.analytics.android.plugin.configs.HZAnalyticsHookConfig
import com.hzdata.analytics.android.plugin.fragment.HZFragmentHookConfig
...
import com.hzdata.analytics.android.plugin.utils.Logger.warn
...
class HZAutoTrackMethodVisitor(
    mv: MethodVisitor,
    methodAccess: Int,
    methodName: String,
    var desc: String,
    ...
    lambdaMethodCells: MutableMap<String, HZAnalyticsMethodCell>,
    ...
    private val mLambdaMethodCells: MutableMap<String, HZAnalyticsMethodCell>
    ...
    override fun visitInvokeDynamicInsn(
        ...
        try {
            ...
            val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                HZAnalyticsHookConfig.LAMBDA_METHODS.get(
                    Type.getReturnType(desc1).descriptor + name1 + desc2
                )
            if (HZAnalyticsMethodCell != null) {
                val it = bsmArgs[1] as Handle
                mLambdaMethodCells[it.name + it.desc] = HZAnalyticsMethodCell
            }
        } catch (e: Exception) {
    ...
    public override fun onMethodEnter() {
        ...
        pubAndNoStaticAccess =
            HZUtils.isPublic(access) && !HZUtils.isStatic(
                access
            )
        protectedAndNotStaticAccess =
            HZUtils.isProtected(access) && !HZUtils.isStatic(
                access
            )
        if (pubAndNoStaticAccess) {
            if (nameDesc == "onClick(Landroid/view/View;)V") {
                ...
                localIds!!.add(third)
            } else if (HZPackageManager.isInstanceOfFragment(classNameAnalytics.superClass)
                && HZFragmentHookConfig.FRAGMENT_METHODS[nameDesc] != null
            ) {
                val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                    HZFragmentHookConfig.FRAGMENT_METHODS[nameDesc]
                localIds = ArrayList()
                val types = Type.getArgumentTypes(HZAnalyticsMethodCell?.agentDesc)
                for (i in 0 until HZAnalyticsMethodCell!!.paramsCount) {
                    val localId = newLocal(types[i])
                    mMethodVisitor.visitVarInsn(HZAnalyticsMethodCell.opcodes[i], i)
                    mMethodVisitor.visitVarInsn(
                        HZUtils.convertOpcodes(
                            HZAnalyticsMethodCell.opcodes.get(i)
                        ), localId
                    )
                ...
                mMethodVisitor.visitVarInsn(ISTORE, secondLocalId)
                localIds!!.add(secondLocalId)
            } else if (HZPackageManager.isTargetMenuMethodDesc(nameDesc)) {
                localIds = ArrayList()
        ...
        if (pluginManager.extension.lambdaEnabled) {
            val lambdaMethodCell: HZAnalyticsMethodCell? = mLambdaMethodCells[nameDesc]
            if (lambdaMethodCell != null) {
                //判断是否是在采样中，在采样中才会处理或者开关打开也统一处理
                if (pluginManager.extension.lambdaParamOptimize || HZAnalyticsHookConfig.SAMPLING_LAMBDA_METHODS.contains(
                        lambdaMethodCell
                    )
                ) {
                    ...
                    val isStaticMethod: Boolean = HZUtils.isStatic(access)
                    localIds = ArrayList()
    ...
    private fun handleCode() {
        if (isHasInstrumented || classNameAnalytics.isHZDataAPI) {
            return
        }

        if (HZPackageManager.isInstanceOfFragment(classNameAnalytics.superClass)) {
            val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                HZFragmentHookConfig.FRAGMENT_METHODS[nameDesc]
            if (HZAnalyticsMethodCell != null) {
...
//                mMethodVisitor.visitVarInsn(ALOAD, 0)
                for (i in 0 until HZAnalyticsMethodCell.paramsCount) {
                    mMethodVisitor.visitVarInsn(
                        HZAnalyticsMethodCell.opcodes[i],
                        localIds!![i]
                    )
                }
                mMethodVisitor.visitMethodInsn(
                    ...
                    HZAnalyticsMethodCell.agentName,
                    HZAnalyticsMethodCell.agentDesc,
                    false
                )
                isHasTracked = true
                return
            ...
            if (lambdaMethodCell != null) {
                ...
                if (!isStaticMethod) {
                    if (lambdaMethodCell.desc.equals("(Landroid/view/MenuItem;)Z")) {
                        ...
                        mMethodVisitor.visitMethodInsn(
                            INVOKESTATIC,
                            HZAnalyticsHookConfig.SENSORS_ANALYTICS_API,
        ...
        if (isAndroidTv && HZPackageManager.isInstanceOfActivity(classNameAnalytics.superClass) && nameDesc == "dispatchKeyEvent(Landroid/view/KeyEvent;)Z") {
            mMethodVisitor.visitVarInsn(ALOAD, 0)
            mMethodVisitor.visitVarInsn(ALOAD, 1)
            mMethodVisitor.visitMethodInsn(
                INVOKESTATIC,
        ...
        if (pluginManager.isModuleEnable(HZModule.REACT_NATIVE) && handleRN()) {
            isHasTracked = true
            return
        }
         ...
         */if (HZPackageManager.isTargetMenuMethodDesc(nameDesc)) {
            mMethodVisitor.visitVarInsn(ALOAD, localIds!![0])
            mMethodVisitor.visitVarInsn(ALOAD, localIds!![1])
            mMethodVisitor.visitMethodInsn(
                INVOKESTATIC,
        ...
        if (classNameAnalytics.interfaces != null && classNameAnalytics.interfaces.size > 0) {
            if (isOnItemClickMethod && classNameAnalytics.interfaces.contains("android/widget/AdapterView\$OnItemClickListener")) {
                ...
                val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                    HZAnalyticsHookConfig.INTERFACE_METHODS
                ...
                for (interfaceName in classNameAnalytics.interfaces) {
                    val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                        HZAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                    if (HZAnalyticsMethodCell != null) {
                        mMethodVisitor.visitVarInsn(ALOAD, localIds!![0])
                        mMethodVisitor.visitMethodInsn(
                            INVOKESTATIC,
                ...
                for (interfaceName in classNameAnalytics.interfaces) {
                    val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
                        HZAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                    if (HZAnalyticsMethodCell != null) {
                        HZAnalyticsMethodCell.visitHookMethod(
                            mMethodVisitor,
                            INVOKESTATIC,
                            HZAnalyticsHookConfig.SENSORS_ANALYTICS_API
                        )
    ...
    private fun handleClassMethod(className: String, nameDesc: String) {
        val HZAnalyticsMethodCell: HZAnalyticsMethodCell? =
            HZAnalyticsHookConfig.CLASS_METHODS.get(className + nameDesc)
        if (HZAnalyticsMethodCell != null) {
            HZAnalyticsMethodCell.visitHookMethod(
                mMethodVisitor,
                INVOKESTATIC,
